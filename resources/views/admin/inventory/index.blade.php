...
@section('content')
  ...
  <div class="bg-white rounded-lg shadow overflow-auto">
    ...
    <tbody>
      ...
      <td>{{ $item->sanPham->masanpham ?? '(Không có)' }}</td> {{-- Mã sản phẩm --}}

      <td class="text-left">{{ $item->sanPham->ten ?? '(Không có)' }}</td>

      <td>
  {{ $item->sanPham->loais->first()->loai ?? '(Không có)' }}
</td>

      <td class="px-4 py-2">
      {{ $item->sanPham->so_luong ?? 0 }}
      </td>
      ...
      <td class="px-4 py-2">
      {{ number_format($item->sanPham->gia_ban ?? 0, 0, ',', '.') }} ₫
      </td>

      <td class="px-4 py-2">
      {{ number_format($item->gianhap ?? 0, 0, ',', '.') }} ₫
      </td>

      <td class="px-4 py-2">
      {{ number_format($item->sanPham->gia_buon ?? 0, 0, ',', '.') }} ₫
      </td>
      </tr>
    @endforeach
    </tbody>
